 ...
 * Complexities: Avg Quicksort = O(nlgn)
		 Avg Heapsort = O(nlgn), unde doar Build Heap Bottom Down constituie O(n)
 * Daca vorbim pe grafic, Overall, Quicksort este mai eficient decat Heapsort si ca numar de atribuiri,
 * si ca numar de comparatii. Difera vizibil pantele celor doi algoritmi pe orice grafic
 * ai lua (atribuiri, comparatii, total), quicksort-ul iesind mereu campion.
 * As indrazni sa afirm ca Quicksort este de doua ori mai eficient decat Heapsort, pe grafic
 */
///HeapSort
int left(int i)
{
...
void maxHeapify(int H[], int n, int n_profiler, int nodStart)
{
    Operation Atrib = p.createOperation("BottomUpAtrib", n_profiler);
    Operation Comp = p.createOperation("BottomUpComp", n_profiler);
    int dreapta;
    if (right(nodStart) > n - 1)
        dreapta = left(nodStart);
    else
        dreapta = right(nodStart);
    int largest = indexOfMax(H, nodStart, left(nodStart), dreapta, n_profiler);
    if (largest != nodStart)
    {
        Atrib.count();
        swap(H[nodStart], H[largest]);
        if (largest <= n / 2 - 1) //pozitia lui n/2-1 este ultimul nod intern care sigur este parinte
            maxHeapify(H, n, n_profiler, largest);
    }
}
...
void HEAPSORT(int H[], int n)
{
    ...
    for (i = n - 1; i >= 1; i--)
    {
        ...
        n--;
        if (n > 1)
            maxHeapify(H, n, n_original, 0);//unde 0 - nodul de pornire in heap
    }
}
void demo_heapsort()
{
...
void demo_quicksort(){
    ...
    listArray(arr, n);
    QUICKSORT(arr, 0, n - 1, n, false, "nume1", "nume2");
    listArray(arr, n);
}
...
void perf()
{
    ...
    p.addSeries("heapsortAVG", "BottomUpAtrib", "BottomUpComp");

    p.createGroup("atribuiri", "atribAVG", "BottomUpAtrib");
    p.createGroup("comparatii", "compAVG", "BottomUpComp");
    p.createGroup("total", "quicksortAVG", "heapsortAVG");
