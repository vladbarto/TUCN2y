 ...
 * Complexities: Avg = O(nlgn)
		 Best = O(nlgn)
		 Worst = O(n^2)
	*Asta ii analiza pt AVG, BEST SI WORST
 * The thing is that per total, asa cum o arata si graficul, average case se apropie de best case.
 * Pe graficul atribuirilor se vede cel mai bine o diferenta în caz mediu, best si worst statistic. De unde deducem că
 * quicksort este un algoritm care se bazează pe mai multe atribuiri decât comparații.
 Best: fiecare partitie imparte array-ul in doua parti egale, pentru ca pivotul este ales drept elementul din mijloc (sau mijloc stanga,
if numar par de elemente in array)
 Worst: Acest caz este generat de alegerea drept pivot a primului sau a ultimului element (it doesn't matter). Array-ul este impartit in
 doua> un array cu un element, si un alt array cu restul de n-1 elememte. => n + (n-1) + (n-2) + ... = O(n^2)
 */
